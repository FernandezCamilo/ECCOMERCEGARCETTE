...
class Carrito {
  ...
  Subtotal() {
    let aux = this.carrito.reduce(
      (acumulador, x) => acumulador + x.precio * x.cantidad,
      0
    )
    return aux
  }
...
const cargarProductos = async () => {
  ...
  data.forEach(element => {
    document
      .getElementById(`btn-zapatilla${element.id}`)
      ?.addEventListener('click', () => {
        if (
          listaCompra
            .GetCarrito()
            .find(zapatilla => zapatilla.id === element.id)
        ) {
          let index = listaCompra
            .GetCarrito()
            .findIndex(zapatilla => zapatilla.id === element.id)
